...
impl Ray3D {
    ...
    pub fn contains_point(&self, point: &Point3D, tolerance: &ToleranceContext) -> bool {
        ...
        if cross_mag >= tolerance.linear { return false; }
    ...
    pub fn is_forward(&self, point: &Point3D, tolerance: &ToleranceContext) -> bool {
        ...
        let dx = self.direction.x(); let dy = self.direction.y(); let dz = self.direction.z();
        let vx = px - ox; let vy = py - oy; let vz = pz - oz;
        (vx*dx + vy*dy + vz*dz) >= -tolerance.linear
    }
    ...
    pub fn distance_to_point_f64(&self, point: &Point3D) -> f64 {
        ...
        let dx = self.direction.x(); let dy = self.direction.y(); let dz = self.direction.z();
        let vx = px - ox; let vy = py - oy; let vz = pz - oz;
        let proj = vx*dx + vy*dy + vz*dz; // direction は単位ベクトル前提
        if proj <= 0.0 { (vx*vx + vy*vy + vz*vz).sqrt() } else {
            // 垂線距離 = |v × d| (d 単位) => sqrt(|v|^2 - proj^2)
            let v_len_sq = vx*vx + vy*vy + vz*vz;
            (v_len_sq - proj*proj).max(0.0).sqrt()
        }
    }
    ...
    pub fn parameter_of_point_f64(&self, point: &Point3D, tolerance: &ToleranceContext) -> Option<f64> {
        ...
        let px = point.x().value(); let py = point.y().value(); let pz = point.z().value();
        let dx = self.direction.x(); let dy = self.direction.y(); let dz = self.direction.z();
        let param = (px - ox)*dx + (py - oy)*dy + (pz - oz)*dz;
    ...
    pub fn closest_point_on_ray(&self, point: &Point3D) -> Point3D {
        ...
        if proj <= 0.0 { return self.origin.clone(); }
        self.evaluate_f64(proj).unwrap()
    }
    ...
    pub fn translate(&self, dx: Scalar, dy: Scalar, dz: Scalar) -> Ray3D { self.translate_f64(dx.value(), dy.value(), dz.value()) }
