...
use crate::geometry3d::{Point3D, Vector3D, BBox3D, Direction3D};
use crate::geometry2d;
use crate::traits::{Circle3D, Circle2D, Direction};
...
impl Circle {
    ...
    pub fn new(center: Point3D, radius: f64, normal: Direction3D, u_axis: Direction3D) -> Self {
        ...
        assert!(dot.abs() < GEOMETRIC_TOLERANCE, "法線ベクトルとu_axisは垂直である必要があります");
        ...
        let v_axis = Direction3D::from_vector(cross_product)
            .expect("v_axisの計算に失敗しました");

        Self { center, radius, normal, u_axis, v_axis }
    }
    ...
    pub fn point_on_plane(&self, point: &Point3D, tolerance: f64) -> bool {
        ...
        let distance = to_point.dot(&Vector3D::new(self.normal.x(), self.normal.y(), self.normal.z())).abs();
        distance <= tolerance
    }
