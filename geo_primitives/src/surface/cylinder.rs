...
impl Cylinder {
    ...
    pub fn axis_direction(&self) -> Vector3D {
        self.axis_vector().normalize().unwrap_or(Vector3D::unit_z())
    }
    ...
    pub fn evaluate(&self, u: f64, v: f64) -> Point3D {
        ...
        // 円形断面上の点
        let circle_point = u_vec * (self.radius * u.cos()) + v_vec * (self.radius * u.sin());
    ...
    pub fn normal(&self, u: f64, _v: f64) -> Vector3D {
        ...
        // 法線は半径方向
        let normal = u_vec * u.cos() + v_vec * u.sin();
