...
#[cfg(test)]
pub mod tests {
    use crate::geometry2d::point::{Point, Point2D, Point2Df};
    use geo_foundation::abstract_types::Scalar;

    #[test]
    fn test_point_f64_creation() {
        // f64版（デフォルト）
        let point_f64: Point<f64> = Point::new(3.0, 4.0);
        assert_eq!(point_f64.x(), 3.0f64);
        assert_eq!(point_f64.y(), 4.0f64);

        // Point2D型エイリアス使用
        let point_alias: Point2D = Point::new(1.0, 2.0);
    ...
    fn test_point_f32_creation() {
        // f32版
        let point_f32: Point<f32> = Point::new(3.0f32, 4.0f32);
        assert_eq!(point_f32.x(), 3.0f32);
        assert_eq!(point_f32.y(), 4.0f32);

        // Point2Df型エイリアス使用
        let point_alias: Point2Df = Point::new(1.0f32, 2.0f32);
    ...
    fn test_point_distance_calculations() {
        // f64での距離計算
        let p1_f64: Point<f64> = Point::new(0.0, 0.0);
        let p2_f64: Point<f64> = Point::new(3.0, 4.0);
        let distance_f64 = p1_f64.distance_to(&p2_f64);
        assert!((distance_f64 - 5.0).abs() < f64::EPSILON);

        // f32での距離計算
        let p1_f32: Point<f32> = Point::new(0.0f32, 0.0f32);
        let p2_f32: Point<f32> = Point::new(3.0f32, 4.0f32);
    ...
    fn test_point_origins() {
        // f64原点
        let origin_f64: Point<f64> = Point::origin();
        assert_eq!(origin_f64.x(), 0.0f64);
        assert_eq!(origin_f64.y(), 0.0f64);

        // f32原点
        let origin_f32: Point<f32> = Point::origin();
