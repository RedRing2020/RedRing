...
impl Triangle2D {
    ...
    /// 頂点を取征E    pub fn vertices(&self) -> &[Point2D; 3] {
        &self.vertices
    }

    /// 頂点の可変参照を取征E    pub fn vertices_mut(&mut self) -> &mut [Point2D; 3] {
        &mut self.vertices
    }
    ...
    /// 点が三角形冁E��にあるかを判定（重忁E��標系を使用�E�E    pub fn contains_point(&self, point: &Point2D) -> bool {
        let (x0, y0) = point2d_to_f64(&self.vertices[0]);
        let (x1, y1) = point2d_to_f64(&self.vertices[1]);
        let (x2, y2) = point2d_to_f64(&self.vertices[2]);
...
/// 3D三角形プリミティチE#[derive(Debug, Clone)]
...
impl Triangle3D {
    ...
    /// 頂点を取征E    pub fn vertices(&self) -> &[Point3D; 3] {
        &self.vertices
    }

    /// 頂点の可変参照を取征E    pub fn vertices_mut(&mut self) -> &mut [Point3D; 3] {
        &mut self.vertices
    }
    ...
    /// 面積を計算（外積を使用�E�E    pub fn area(&self) -> f64 {
    ...
    /// 法線�Eクトルを計箁E    pub fn normal(&self) -> Vector3D {
        let (x0, y0, z0) = point3d_to_f64(&self.vertices[0]);
        let (x1, y1, z1) = point3d_to_f64(&self.vertices[1]);
        let (x2, y2, z2) = point3d_to_f64(&self.vertices[2]);
...
mod tests {
    ...
    fn test_triangle2d_contains_point() {
        ...
        let p3 = Point2D::new(1.0, 2.0);
        let triangle = Triangle2D::new(p1, p2, p3);

        let inside_point = Point2D::new(1.0, 0.5);
        let outside_point = Point2D::new(2.0, 2.0);
