...
- `foundation/analysis` は純粋計算層として扱い、`tracing` 等のログ依存を直接導入しない
- 行列計算・数値計算のログは呼び出し側（`geo_*` / `viewmodel_*` / `render` / `app`）で記録する
- 必要な可観測性は、analysis側で「メトリクス値を返すAPI」を追加し、ログ出力は上位層で実施する
- これにより依存チェック違反リスクを回避し、層分離を維持する
...
本Issueでは「まず負荷抑制を達成しつつ、将来の疎結合化コストを最小化する」ことを目的に、
以下の段階移行を採用する。

### Phase 0（完了）: 最小限の負荷抑制

- 既定フィルタの引き締め（高頻度層を `warn` 基調へ）
- フレーム系ログを `trace/debug` へ降格
- 間引き（Nフレーム毎）の導入

到達条件:
- デフォルト起動でフレーム毎ログが出ない
- `RUST_LOG` で必要時のみ詳細化できる

### Phase 1（次ステップ）: Foundation API 先行固定

`foundation/logging_foundation`（仮）を追加し、まず API と運用規約を固定する。
この段階では全クレート移行を完了させなくてよい。

最小API:
- 初期化API（既定フィルタ + `RUST_LOG` 尊重）
- レート制御API（Nフレーム / 時間間隔）
- 共通マクロ（フレームログ、間引きログ）

到達条件:
- 新規ログ追加時は Foundation API を使う規約に統一
- 直接 `tracing_subscriber` 初期化の新規実装を禁止

### Phase 2（段階移行）: 呼び出し点の集約

優先度の高い高頻度層から順に、既存ログ呼び出しを Foundation API へ移行する。
...
到達条件:
- 高頻度ログの主要呼び出しが共通マクロ経由になる
- 間引きロジックの重複実装が撤去される

### Phase 3（統制）: 直接依存の削減

- 各実装クレートの `tracing_subscriber` 直接利用を段階撤去
- ログ方針逸脱を CI で検出（簡易lint/grep ベースで可）

到達条件:
- ログ初期化経路が原則1箇所に集約される
- ログレベル運用がドキュメントと実装で一致する
...
- Foundation配下新規クレート作成の承認取得
- 依存ルール影響の確認（既存アーキテクチャチェックに適合）

### Gate B（Phase 2 展開条件）

- Foundation API の最小仕様が固まっている
- 既存実装で性能退行がない
