...
## 🔟 #751 CAM テッセレーション品質制御

本節は Issue #751 の設計と計測結果を記録する。

### 10.1 方式

- `cam_algorithms::tessellation` のテンソル積パラメータ格子方式を維持する
- `TessellationLimits` に品質条件を追加する（いずれも任意、既定は未設定）
  - `max_aspect_ratio`: 三角形アスペクト比（最長辺 / (2√3 × 内接円半径)、正三角形で 1）の上限
  - `max_edge_length`: 三角形の辺長（3D）の上限
- 各セルは短い方の対角線で 2 三角形に分割する
- 品質条件を超えるセルは、超える方向のパラメータ区間を等分する（格子単位の再分割のため T 字接続は発生しない）
  - 最大辺長: 辺が上限を超える方向を `ceil(辺長 / 上限)` 等分。対角線が超える場合は長い方の辺を二分して再検証
  - アスペクト比: 長い方向を `floor(長辺 / 短辺)` 等分
  - 辺長比 2 未満の長方形セルの三角形アスペクト比は約 1.49 以下のため、辺長比 2 未満で上限を超えるセルはせん断が原因と判定し、`convergence_failure` とする（等分では改善しない）
- 反復上限・頂点数上限内に条件を満たせない場合は `convergence_failure`
...
| `mold_finish` | アスペクト比 ≤ 4 | 頂点数上限超過（`convergence_failure`） | — | — |
| `mold_finish` | 辺長 ≤ 工具半径 | 137,216 | 22.4 | 313ms |

2 方向に曲がるドーム形状（40mm）では、いずれの品質条件でも三角形数・アスペクト比は変化しない。
計測は `tessellation_quality_measurement`（`#[ignore]`）で再現できる。

### 10.3 判断

- 細長三角形は drop-cutter の性能を劣化させない。経路生成時間は三角形数が支配的で、品質制御なしが最速となる
- 逆オフセットは要素ごとに厳密に計算するため、細長三角形でも CL は品質制御後と弦誤差内で一致する（テストで確認）
- CAM 精度でのアスペクト比制御は三角形数が数百倍となり、`mold_finish` では頂点数上限内に収まらない
- 以上から、品質条件は既定で無効とし、solver の既定動作は変更しない
- 形状品質を必要とする後続処理（三角形限定トポロジー、シミュレーション比較等）が必要に応じて有効化する
