...
**関連Issue**: #206（Octree実装）, #207（Octree可視化）
...
- 実装上の操作は `o`（深さ送り） / `Shift+O`（深さアニメーション）を採用
- 「スクリーンショット撮影」は新規機能要件ではなく、検証エビデンスとして扱う
- エビデンスは #206 作業時の確認結果を参照し、可視化機能の追加要件には含めない
- 干渉検知トリガーでの自動保存は、必要になった時点で別Issue化する
