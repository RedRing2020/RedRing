...
トレランスの選択は、関数名ではなく「最終比較している量の次元」で決定する。

1. 最終比較量が距離（length）なら距離トレランスを使う。
2. 最終比較量が角度（radian）なら角度トレランスを使う。
3. 最終比較量が無次元量（正規化済み内積/外積誤差）なら無次元しきい値を使う。

補足:

- 途中計算に角度や外積が現れても、最終的に距離へ変換して比較する場合は距離トレランスを採用する。
- 関数名に `angle` を含んでも、実際の比較対象が `magnitude` / `distance` なら距離トレランスを使う。

## 幾何判定における正規化ルール

スカラ三重積のような「距離以外の量」を距離トレランスと比較する場合は、意味量（距離）へ正規化してから比較する。

1. 交差/ねじれ判定で三重積を使う場合、`|triple| / |cross|` で直線間最短距離へ変換して比較する。
2. `|triple| <= tolerance` のような未正規化比較を、距離トレランス判定の正本にしない。
3. 正規化で分母ゼロ近傍になる場合は、先に平行判定（またはゼロ近傍ガード）を実施する。

## `geo_algorithms` unit tests のトレランス運用補足

`geo_algorithms` の unit テストで新規ケースを追加する場合は、既存慣例に合わせて標準トレランス関数を優先する。

1. `let tolerance = 1e-9;` の直接記述を常用しない。
2. 既存ファイルに `standard_distance_tol()` 等の標準関数がある場合はそれを使う。
3. 標準値より緩い/厳しい値が必要な場合は、理由をテスト内コメントで明示する。
