...
- `dev/architecture/ISSUE_*.md` / `dev/foundation/ISSUE_*.md` のうち、対応Issueが `closed` のものは本ディレクトリへ移動する
- `open` の Issue ドキュメントは従来どおり現行ディレクトリ（`dev/architecture` 等）で管理する
- アーカイブ後もファイル名は `ISSUE_<番号>_...` を維持する
