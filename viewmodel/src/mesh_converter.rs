...
//! geo_primitives::TriangleMesh3D を GPU レンダリング用の頂点データに変換します。
...
/// CCWと法線の整合性を検証（簡易版）
fn validate_ccw_normal_consistency<T: Scalar>(
    edge1: &Vector3D<T>,
    edge2: &Vector3D<T>,
    computed_normal: &Vector3D<T>,
) -> bool {
    // CCW順序での外積による法線計算
    ...
    // f64変換して比較（簡易版）
    dot_product.to_f64() > 0.9
}

/// TriangleMesh3D を GPU用頂点データのみに変換（render用の簡易版）
pub fn triangle_mesh_to_vertices<T: Scalar>(mesh: &TriangleMesh3D<T>) -> Vec<VertexData> {
...
pub fn triangle_mesh_to_vertices_with_indices<T: Scalar>(
