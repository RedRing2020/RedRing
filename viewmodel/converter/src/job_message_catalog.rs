use crate::message_catalog::{MessageCatalog, UiLocale, UiMessage};

pub struct JobMessageCatalog;

impl MessageCatalog for JobMessageCatalog {
    fn template(&self, locale: UiLocale, key: &str) -> &'static str {
        match locale {
            UiLocale::Ja => resolve_ja_template(key),
            UiLocale::En => resolve_en_template(key),
        }
    }
}

pub fn resolve_job_message(locale: UiLocale, message: &UiMessage) -> String {
    crate::message_catalog::resolve_message(&JobMessageCatalog, locale, message)
}
