...
impl From<JobOutputValidity> for ArtifactValidityDto {
...
pub fn status_to_progress_percent(status: JobStatus) -> f32 {
    match status {
        ...
        JobStatus::NeedsRecompute => 0.0,
        JobStatus::Succeeded | JobStatus::Failed | JobStatus::Canceled => 100.0,
    }
}
...
pub fn job_event_to_progress_dto(event: &JobEvent) -> Option<JobProgressDto> {
    match event {
        ...
        JobEvent::ArtifactReady { job_id, result_ref } => Some(JobProgressDto {
            job_id: job_id.0,
            status: None,
        ...
        JobEvent::Completed {
            ...
            message: if *status == JobStatus::Failed {
                Some(UiMessage {
        ...
        JobEvent::MarkedNeedsRecompute { job_id, by_job_id } => Some(JobProgressDto {
            job_id: job_id.0,
            ...
            message: Some(UiMessage {
                ...
                args: vec![UiMessageArg {
                    ...
                    value: by_job_id.0.to_string(),
                }],
            }),
        }),
        JobEvent::GroupProgressUpdated { .. } => None,
    }
}
...
fn active_artifact_dto(record: &JobRecord) -> Option<ArtifactRefDto> {
    record
        .output_history
        .iter()
...
fn job_type_key(job_type: JobType) -> &'static str {
    match job_type {
        JobType::CamProcess => "job.type.cam_process",
        JobType::CuttingSimulation => "job.type.cutting_simulation",
    }
}
...
fn status_key(status: JobStatus) -> &'static str {
    match status {
        ...
        JobStatus::NeedsRecompute => "needs_recompute",
...
mod tests {
    ...
    use job_runtime::{JobId, JobOutputRecord, RetryPolicy};
    ...
    fn sample_record(status: JobStatus) -> JobRecord {
        JobRecord {
            ...
            output_history: vec![JobOutputRecord {
    ...
    fn progress_can_be_estimated_from_status() {
        let dto = job_record_to_summary_dto(&sample_record(JobStatus::Running), None);
        assert_eq!(dto.progress_percent, 50.0);
    }

    #[test]
    fn artifact_is_wrapped_by_option_artifact_ref_dto() {
        let dto = job_record_to_summary_dto(&sample_record(JobStatus::Succeeded), None);
