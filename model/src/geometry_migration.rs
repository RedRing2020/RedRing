...
pub use crate::geometry_adapter::{
    Direction3D, Normalize, Normed, Point2D, Point3D, Vector2D, Vector3D,
};

#[cfg(not(feature = "use_geo_core"))]
pub use geo_foundation::{
    geometry2d::{point::Point as Point2D, vector::Vector as Vector2D},
    geometry3d::{
        direction::Direction as Direction3D, point::Point as Point3D, vector::Vector as Vector3D,
    },
};

#[cfg(not(feature = "use_geo_core"))]
pub use crate::geometry_trait::{normalize::Normalize, normed::Normed};

/// モジュール互換性のための re-export
pub mod geometry2d {
    #[cfg(feature = "use_geo_core")]
    pub use crate::geometry_adapter::{Point2D as Point, Vector2D as Vector};

    #[cfg(not(feature = "use_geo_core"))]
    pub use geo_foundation::geometry2d::{point::Point, vector::Vector};

    // Direction は geo_foundation から取得
    pub use geo_foundation::geometry2d::direction::Direction;
}

pub mod geometry3d {
    #[cfg(feature = "use_geo_core")]
    pub use crate::geometry_adapter::{
        Direction3D as Direction, Point3D as Point, Vector3D as Vector,
    };

    #[cfg(not(feature = "use_geo_core"))]
    pub use crate::geometry::geometry3d::{direction::Direction, point::Point, vector::Vector};
}
...
pub mod test_helpers {
    ...
    pub fn compare_vector_operations() {
        ...
        #[cfg(feature = "use_geo_core")]
        {
            ...
            assert!(
                (legacy_cross.x() - geo_cross.x()).abs() < 1e-10,
                "Cross product X mismatch"
            );
            assert!(
                (legacy_cross.y() - geo_cross.y()).abs() < 1e-10,
                "Cross product Y mismatch"
            );
            assert!(
                (legacy_cross.z() - geo_cross.z()).abs() < 1e-10,
                "Cross product Z mismatch"
            );
        }
    }
}
