...
pub struct Ellipse2 {
    center: Point2,
    major_axis: Direction2, // 長軸方向（正規化済み）
    major_radius: f64,
    minor_radius: f64,
}

impl Ellipse2 {
    pub fn new(center: Point2, major_axis: Direction2, major_radius: f64, minor_radius: f64) -> Self {
        Self {
            center,
            major_axis,
    ...
    pub fn tangent(&self, theta: f64) -> Direction2 {
        ...
        Direction2::new(dx, dy)
    }

    /// 法線方向（中心から外向き）
    pub fn normal(&self, theta: f64) -> Direction2 {
        let p = self.evaluate(theta);
        Direction2::new(p.x - self.center.x, p.y - self.center.y)
    }

    /// 線分との交差点を離散近似で求める
    pub fn intersection_with_line(&self, line: &Line2) -> Vec<Point2> {
...
mod tests {
    ...
    fn test_ellipse_line_intersection_two_points() {
        let ellipse = Ellipse2::new(Point2::new(0.0, 0.0), Direction2::new(1.0, 0.0), 5.0, 3.0);
        let line = Line2::new(Point2::new(-6.0, 0.0), Point2::new(6.0, 0.0));
