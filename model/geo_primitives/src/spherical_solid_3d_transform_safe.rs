...
/// SphericalSolid3D の安全な変換操作
impl<T: Scalar + GeometricTolerance> SphericalSolid3D<T> {
    /// 安全な均一スケール（原点中心）
    ///
    /// # 引数
    /// * `factor` - スケール倍率（正の値のみ）
    ...
    /// 安全な半径のみスケール（中心・座標系固定）
    ///
    /// # 引数
    /// * `factor` - 半径のスケール倍率（正の値のみ）
    ///
    /// # 戻り値
    /// * `Ok(SphericalSolid3D)` - 半径スケール後の球固体
    /// * `Err(TransformError)` - 無効なスケール倍率または結果
    pub fn safe_scale_radius(&self, factor: T) -> Result<Self, TransformError> {
        ...
        .ok_or(TransformError::InvalidGeometry(
            "半径スケール後の球固体の作成に失敗しました".to_string(),
        ))
    }
    ...
    pub fn safe_with_radius(&self, new_radius: T) -> Result<Self, TransformError> {
        ...
        .ok_or(TransformError::InvalidGeometry(
            "新しい半径での球固体の作成に失敗しました".to_string(),
        ))
    }
