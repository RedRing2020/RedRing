//! ConicalSolid3D Analysis Matrix/Vector統合変換実装
//!
//! Analysis Matrix4x4を直接使用した効率的な3D円錐ソリッド変換
//! Point3D/Vector3D Analysis Transform パターンを基盤とする統一実装
//! 3D円錐ソリッドの特性（中心点・軸・参照方向・半径・高さ）を考慮したMatrix変換
...
/// ConicalSolid3D用Analysis Matrix4x4変換モジュール
pub mod analysis_transform {
    ...
    pub fn transform_conical_solid_3d<T: Scalar>(
        conical_solid: &ConicalSolid3D<T>,
        matrix: &Matrix4x4<T>,
        ...
        let new_radius = conical_solid.radius() * scale_factor;
        let new_height = conical_solid.height() * scale_factor;
        ...
        .ok_or_else(|| {
            TransformError::InvalidGeometry(
                "Failed to create transformed ConicalSolid3D".to_string(),
            )
        })
    }
...
/// ConicalSolid3D用AnalysisTransform3Dトレイト実装
impl<T: Scalar> AnalysisTransform3D<T> for ConicalSolid3D<T> {
    type Matrix4x4 = Matrix4x4<T>;
    type Angle = Angle<T>;
    type Output = ConicalSolid3D<T>;

    /// Matrix4x4による直接変換
    fn transform_point_matrix(&self, matrix: &Self::Matrix4x4) -> Self::Output {
        analysis_transform::transform_conical_solid_3d(self, matrix)
            .expect("ConicalSolid transformation should be valid")
    }
...
mod tests {
    ...
    fn create_test_conical_solid() -> ConicalSolid3D<f64> {
        ConicalSolid3D::new_standard(
            Point3D::new(1.0, 2.0, 3.0), // center
            2.0,                         // radius
            4.0,                         // height
        )
        .unwrap()
    ...
    fn test_analysis_translation() {
        let solid = create_test_conical_solid();
        let translation = Vector3::new(5.0, 3.0, 1.0);

        let result = solid.translate_analysis(&translation).unwrap();

        // 中心点が移動することを確認
        let expected_center = Point3D::new(6.0, 5.0, 4.0);
        ...
        // 軸と参照方向は変わらない
        assert!((result.axis().x() - solid.axis().x()).abs() < f64::EPSILON);
        assert!((result.axis().y() - solid.axis().y()).abs() < f64::EPSILON);
        assert!((result.axis().z() - solid.axis().z()).abs() < f64::EPSILON);

        // 半径と高さは変わらない
        assert!((result.radius() - solid.radius()).abs() < f64::EPSILON);
        assert!((result.height() - solid.height()).abs() < f64::EPSILON);
    }
    ...
    fn test_analysis_rotation() {
        let solid = create_test_conical_solid();
        let center_solid =
            ConicalSolid3D::new_standard(Point3D::new(0.0, 0.0, 0.0), 1.0, 1.0).unwrap();
        let axis = Vector3::new(1.0, 0.0, 0.0); // x軸回転
        let angle = Angle::from_degrees(90.0);

        let result = solid.rotate_analysis(&center_solid, &axis, angle).unwrap();

        // 90度X軸回転後の中心点確認
        let expected_center = Point3D::new(1.0, -3.0, 2.0);
        ...
        // 軸方向も回転される（Z軸(0,0,1)をX軸周りに90度回転すると(0,-1,0)になる）
    ...
    fn test_analysis_scale() {
        ...
        let result = solid
            .scale_analysis(&center_solid, scale_x, scale_y, scale_z)
            .unwrap();

        // スケール変換後の中心点確認
        let expected_center = Point3D::new(2.0, 6.0, 12.0);
        assert!((result.center().x() - expected_center.x()).abs() < f64::EPSILON);
        assert!((result.center().y() - expected_center.y()).abs() < f64::EPSILON);
        assert!((result.center().z() - expected_center.z()).abs() < f64::EPSILON);

        // 半径と高さがスケールされることを確認
        // 軸がZ方向なので、半径は主にX,Y成分、高さはZ成分でスケールされる
        assert!(result.radius() > solid.radius());
        assert!(result.height() > solid.height());
    }
    ...
    fn test_analysis_uniform_scale() {
        ...
        let result = solid
            .uniform_scale_analysis(&center_solid, scale_factor)
            .unwrap();
        ...
        assert!((result.radius() - solid.radius() * scale_factor).abs() < f64::EPSILON);
        assert!((result.height() - solid.height() * scale_factor).abs() < f64::EPSILON);
    }
    ...
    fn test_analysis_composite_transform() {
        ...
        let result = solid
            .apply_composite_transform(
                Some(&translation),
                Some((&rotation_center_solid, &rotation_axis, rotation_angle)),
                Some(scale_factors),
            )
            .unwrap();
        ...
        let expected_center = Point3D::new(3.0, 5.0, 7.0); // (1*2+1, 2*2+1, 3*2+1)
        assert!((result.center().x() - expected_center.x()).abs() < f64::EPSILON);
        assert!((result.center().y() - expected_center.y()).abs() < f64::EPSILON);
        assert!((result.center().z() - expected_center.z()).abs() < f64::EPSILON);
    }
    ...
    fn test_zero_scale_error() {
        ...
        let result = solid.scale_analysis(&center_solid, 0.0, 1.0, 1.0);
    ...
    fn test_zero_rotation_axis_error() {
        ...
        let result = solid.rotate_analysis(&center_solid, &zero_axis, angle);
    ...
    fn test_transform_point_matrix() {
        ...
        let result = solid.transform_point_matrix(&matrix);
