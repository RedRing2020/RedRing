...
mod tests {
    ...
    fn test_default_infinite_line_3d() {
        ...
        assert_eq!(line.point_internal(), Point3D::origin());
        assert_eq!(line.direction_internal().x(), 1.0);
        assert_eq!(line.direction_internal().y(), 0.0);
        assert_eq!(line.direction_internal().z(), 0.0);
    }
    ...
    fn test_analysis_transform_translate() {
        ...
        assert_eq!(result.point_internal().x(), 1.0);
        assert_eq!(result.point_internal().y(), 2.0);
        assert_eq!(result.point_internal().z(), 3.0);
        // 方向ベクトルは変わらない
        assert_eq!(result.direction_internal().x(), 1.0);
        assert_eq!(result.direction_internal().y(), 0.0);
    ...
    fn test_analysis_transform_rotate() {
        ...
        assert!((result.point_internal().y() - 1.0).abs() < TOLERANCE);
        assert!((result.point_internal().z() - 0.0).abs() < TOLERANCE);
        // 方向ベクトル (1,0,0) は (0,1,0) になる
        assert!((result.direction_internal().x() - 0.0).abs() < TOLERANCE);
        assert!((result.direction_internal().y() - 1.0).abs() < TOLERANCE);
        assert!((result.direction_internal().z() - 0.0).abs() < TOLERANCE);
    }
    ...
    fn test_analysis_transform_scale() {
        ...
        assert_eq!(result.point_internal().y(), 3.0);
        assert_eq!(result.point_internal().z(), 4.0);
    ...
    fn test_analysis_transform_uniform_scale() {
        ...
        assert_eq!(result.point_internal().y(), 2.0);
        assert_eq!(result.point_internal().z(), 2.0);
        // 方向ベクトルは変わらない（均等スケール）
        assert_eq!(result.direction_internal().x(), 1.0);
        assert_eq!(result.direction_internal().y(), 0.0);
        assert_eq!(result.direction_internal().z(), 0.0);
    ...
    fn test_composite_transform() {
        ...
        // 複合変換の結果を検証（詳細は実装依存）
        assert!(result.point_internal().x().is_finite());
        assert!(result.point_internal().y().is_finite());
        assert!(result.point_internal().z().is_finite());
    }
    ...
    fn test_transform_point_matrix() {
        ...
        assert_eq!(result.point_internal().x(), 3.0);
        assert_eq!(result.point_internal().y(), 3.0);
        assert_eq!(result.point_internal().z(), 4.0);
        assert_eq!(result.direction_internal().x(), 1.0);
        assert_eq!(result.direction_internal().y(), 0.0);
