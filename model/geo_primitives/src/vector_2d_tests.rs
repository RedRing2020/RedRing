...
fn test_transform_vector_trait() {
    ...
    impl crate::vector_2d_transform::TransformVector2D<f64> for SimpleMatrix {
        fn transform_vector_2d(&self, vector: &Vector2D<f64>) -> Vector2D<f64> {
            // 単純なスケール変換
            Vector2D::new(vector.x() * 2.0, vector.y() * 2.0)
        }
    }
...
fn test_transform_point_trait() {
    ...
    impl crate::vector_2d_transform::TransformPoint2D<f64> for SimpleMatrix {
        fn transform_point_2d(&self, point: &Point2D<f64>) -> Point2D<f64> {
            // 単純な平行移動
            Point2D::new(point.x() + 1.0, point.y() + 1.0)
        }
    }
