...
use crate::{Cylinder3D, Point3D, TriangleMesh3D, Vector3D};
...
impl<T: Scalar> Cylinder3D<T> {
    ...
    /// 円柱を平行移動
    pub fn translate(&self, offset: Vector3D<T>) -> Cylinder3D<T> {
        let new_center = Point3D::new(
            self.center().x() + offset.x(),
            self.center().y() + offset.y(),
            self.center().z() + offset.z(),
        );
        Cylinder3D::new(new_center, self.axis(), self.radius(), self.height()).unwrap()
    }
}
