...
pub mod analysis_transform {
    ...
    pub fn transform_cylindrical_surface_3d<T: Scalar>(
        ...
        // 中心点を変換
        let center_vec: Vector3<T> = cylindrical_surface.center().into();
        let transformed_center_vec = matrix.transform_point_3d(&center_vec);
        let new_center: Point3D<T> = transformed_center_vec.into();

        // 軸方向を変換
        let axis_vec: Vector3<T> = cylindrical_surface.axis().as_vector().into();
        ...
        let ref_dir_vec: Vector3<T> =
            cylindrical_surface.ref_direction().as_vector().into();
