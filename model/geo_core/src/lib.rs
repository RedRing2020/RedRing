...
//!
//! 交差判定等、全幾何の組み合わせが必要な実装用クレート
//! 数値安定性・比較ロジック・低レベル補助機能を提供
//!
//! このクレートは **許容誤差 (tolerance)** と **ロバスト幾何判定** を中心とした
//! 最小構成の計算インフラを提供します。形状プリミティブ（線分 / 平面 / 円など）の
//! 正典定義は `geo_primitives` クレートへ段階的に移管され、`geo_core` は
//! 「数値安定性・比較ロジック・低レベル補助」のみに収束していきます。
//!
//! 詳細なアーキテクチャ設計は dev/architecture/ を参照
//!
...
//! - [Phase] f64 正典プリミティブ: `geo-primitives::f64geom::*` に集約 (進行中)
...
//! 1. f64 正典型公開 → 旧型は feature gate + deprecated
...
// geo_coreは共通の計算ロジック・アルゴリズムを提供
// Transform実装は各クレートで独自に実装
