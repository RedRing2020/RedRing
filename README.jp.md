...
| `r` | カメラリセット |
| `t` | 標準CAD視点へ復帰 |
| `w` | 表示モード切替（CAM表示中は Wire/Solid 切替） |
...
| ---- | ---- |
| `p` | カッターパスのみ表示（色分けライン） |
| `Shift+B` | ボールエンドミルのCAMシミュレーション可視化 |
| `Shift+F` | フラットエンドミルのCAMシミュレーション可視化 |
| `k` | スナップショット次フレーム |
| `j` | スナップショット前フレーム |
| `Space` | 自動再生 / 一時停止 |
| `Shift+J` | 停止して先頭フレームへ戻る |
| `+` / `-` | 再生速度変更（倍率はタイトル表示） |
| `Shift+S` | 設定パネル表示切替 |
...
**⚠️ 重要：Git Pre-commit フックは初期セットアップ時に設定済みです**

セットアップ後は、`git commit` 時に自動的に以下を実行：
