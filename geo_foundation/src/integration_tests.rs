/// geo_foundation統合テスト
/// 
/// Scalar trait、Angle、Circle実装の統合動作確認を行います。
/// f32/f64両方での動作と相互運用性をテストします。

use crate::{Scalar, Angle, Circle, Circle2D, Circle2DImpl, Point2D, Vector2D, Vector3D};
...
mod integration_tests {
    ...
    fn test_angle_f32_operations() {
        ...
        assert!((normalized_degrees - 90.0).abs() < 1e-4, 
                "Expected ~90°, got {}°", normalized_degrees);
    ...
    fn test_edge_cases() {
        ...
        // 1点の境界円（ゼロ半径円）
        let single_point_circle = Circle2DImpl::<f64>::bounding_circle(&[Point2D::new(1.0, 2.0)]).unwrap();
    ...
    fn test_vector_point_conversion_workflow() {
        ...
        // ベクトル演算（重心計算）
        let centroid_vector = vectors.iter()
            .fold(Vector2D::zero(), |acc, &v| acc + v) / (vectors.len() as f64);
