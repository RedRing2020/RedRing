...
impl PoissonDiskSampler { pub fn new(radius:f64)->Self { Self { radius, k:30 } }
    pub fn sample_uniform_2d(&self, bounds:(Point2D,Point2D), initial_seed:Option<Point2D>) -> Vec<Point2D> { let mut rng=SimpleRng::new(42); let mut points=Vec::new(); let mut active=Vec::new(); let first = initial_seed.unwrap_or_else(|| { let cx=(bounds.0.x().value()+bounds.1.x().value())*0.5; let cy=(bounds.0.y().value()+bounds.1.y().value())*0.5; Point2D::from_f64(cx,cy)}); points.push(first); active.push(0); while !active.is_empty() { let idx=(rng.next_f64()*active.len() as f64) as usize; let pidx = active[idx]; let p = points[pidx]; let mut found=false; for _ in 0..self.k { let angle=rng.next_f64()*2.0*std::f64::consts::PI; let distance=self.radius*(1.0 + rng.next_f64()); let nx=p.x().value()+distance*angle.cos(); let ny=p.y().value()+distance*angle.sin(); let np=Point2D::from_f64(nx,ny); if self.is_valid_point(&np,&points,bounds) { points.push(np); active.push(points.len()-1); found=true; break; } } if !found { active.swap_remove(idx); } } points }
    fn is_valid_point(&self, p:&Point2D, existing:&[Point2D], bounds:(Point2D,Point2D)) -> bool { if p.x().value()<bounds.0.x().value() || p.x().value()>bounds.1.x().value() || p.y().value()<bounds.0.y().value() || p.y().value()>bounds.1.y().value() { return false; } for e in existing { if e.distance_to(p).value() < self.radius { return false; } } true }
}
