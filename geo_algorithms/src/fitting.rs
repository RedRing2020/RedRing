...
impl LeastSquaresFitter {
    ...
    for p in points { let x=p.x(); let y=p.y(); let x2=x*x; let y2=y*y; s_x+=x; s_y+=y; s_x2+=x2; s_y2+=y2; s_xy+=x*y; s_x3+=x2*x; s_y3+=y2*y; s_x1y2+=x*y2; s_x2y1+=x2*y; }
    ...
    for p in points { let x=p.x(); let y=p.y(); sum_x+=x; sum_y+=y; sum_xy+=x*y; sum_x2+=x*x; }
